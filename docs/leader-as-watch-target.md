...
### 문서
7. `.claude/commands/watch.md` / `Resources/CodexPrompts/watch.md` — "leader 제외"를 "worker가 있으면 leader 제외; worker가 0이면 leader fallback(또는 `--target leader` 명시)"로 갱신. `--target`의 `leader` 값과 fallback 동작을 옵션 표·예시에 추가.
