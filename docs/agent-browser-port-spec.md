...
This document tracks implemented behavior and remaining parity gaps for the term-mesh browser port.

## Goals

1. Provide an LLM-friendly browser automation API in term-mesh with stable handles.
...
1. `./scripts/run-tests-v1.sh` passes on `term-mesh-vm`.
2. `./scripts/run-tests-v2.sh` passes on `term-mesh-vm`.
...
## term-mesh Target API (v2)

### Already Present in term-mesh
...
6. `browser.tab.*` compatibility aliases mapped to term-mesh surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in term-mesh context)
...
Primary form:
```bash
term-mesh browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
term-mesh browser <surface-id> <agent-browser-style-command...>
```

Agent discovery:
```bash
term-mesh identify
term-mesh capabilities
term-mesh browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
term-mesh trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/term-mesh.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/term-mesh.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for term-mesh browser parity (different transport/runtime).

### Implemented term-mesh Browser Suites
...
1. `term-mesh browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is term-mesh-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `term-mesh browser` or gate them behind a second rollout phase.
