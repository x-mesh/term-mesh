...
## Goal
Ensure commands run through the term-mesh Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `TERMMESH_TAB_ID` / `TERMMESH_SURFACE_ID` / `TERMMESH_WORKSPACE_ID` when flags omitted).
