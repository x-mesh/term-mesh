...
- PARTIAL — security 실검증 core(trust/CAS/path-sandbox suites)는 통과. 이전의 "security fault suite 11/11" 주장에서 fault 미주입 fixture 2건(packet-loss/sleep-wake)은 연극이라 제거했고, 실제 fault-injection e2e는 deferred(R16 참조). benchmark smoke threshold는 virtual 측정임(R14 참조).
...
| R14 Scale | PARTIAL | `benchmark-smoke.json`: virtual 50 GiB, 1,000,000 files, sparse allocated 0 B, no-change path-list 0 B, 90% resume retransmit 10%. ⚠ The **7,814 MiB/s** figure is a VIRTUAL throughput derived from a fixed 256 MiB in-memory hash loop, NOT a measured network/disk transfer — treat it as a wiring smoke, not a scale measurement. `reconcile_resume::million_entry_source_streams_to_persisted_index_and_no_change_has_zero_paths`(real 1M-entry stream)도 통과. | Nonblocking deferred (실측 미완): 최소 10 GiB incompressible full run과 실제 1 GiB network interruption artifact. 실제 blob-saturation throughput은 여전히 미측정. |
...
| R16 Security verification | PARTIAL | 실검증 core는 `trust_rotation`, `cas_integrity`, `path_sandbox`/`secure_sqlite` suites다 (forged/stale/revoked/replay/downgrade/cross-project/path/symlink/collision/decompression/disk/SQLite/CAS — 실제 adversarial inputs). `trust_rotation::dek_wrap_binds_device_epoch_and_rotation_journal_resumes_every_phase`도 통과. ⚠ 이전 판에 있던 "packet-loss/sleep-wake" 항목은 **fault를 주입하지 않고 결정론적 fixture 상수만 재확인**하던 연극 테스트였고 제거됨(`security_fault_suite.rs`의 fake `packet_loss_and_sleep_wake...` 삭제). 따라서 이 두 위협은 R16 실검증에 포함되지 않는다. | Nonblocking deferred: real macOS user-presence prompt/cancel, approve→revoke→rotate real Keychain UI drill, **그리고 실제 packet-loss/sleep-wake fault 주입 e2e**. |
