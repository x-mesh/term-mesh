...
9. `project.presentation.repair.v1` gates explicit inspection and administrative removal of stale durable manifests. A host may classify a record as stale only after every referenced surface is absent in two bounded observations on the same authenticated connection. Repair revalidates the exact Project ID, revision, token, and surface absence, writes a complete recoverable backup, removes only that manifest, and never mutates daemon workspaces. Owners continue to use normal `project.presentation.v1` deletion; the administrative path exists only for owner mismatch.
