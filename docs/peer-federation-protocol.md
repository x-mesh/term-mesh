...
10. `relay.telemetry.v1` is client-opt-in. A capable GUI host sends at most one numeric-only `RelayTelemetry` sample per configured cadence to an interested ready session. The sample contains only monotonic/cumulative counters and 16-byte surface IDs: it must not include PTY payload, path, title, or user metadata. A viewer correlates only an exact attached surface ID. `host_aggregate_dropped_*` aggregates every host attachment for that surface and is not causal evidence for one viewer; use that viewer's `receiver_gap_*` counters for viewer-specific loss. A daemon must not advertise this capability until it can send samples. Live comparison requires both viewer and GUI host to run a release containing this capability; do not update or restart an incident host merely to enable development observation.
