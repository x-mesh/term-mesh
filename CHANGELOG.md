...
- **자동화 명령이 처리되는 동안 새로 연 창이 앞으로만 나오고 활성화되지 않던 문제** — 앱은 자동화 명령이 실행되는 동안 창이 저절로 활성화되지 않도록 막습니다. 그런데 명령이 결과를 기다리는 시간까지 실행 중으로 세어, 그 사이 사용자가 직접 연 창도 자동화가 연 것처럼 취급됐습니다. 기다리는 동안에는 그 표시를 해제합니다. (`Sources/TerminalController.swift`)

## [0.217.0] - 2026-08-30

**명령 팔레트를 열자마자 입력해도 글자가 사라지지 않습니다.**

### Fixed

- **팔레트를 열고 바로 친 글자가 사라지던 문제** — 팔레트가 화면에 뜬 뒤 키보드 입력을 넘겨받기까지 평균 47밀리초, 길게는 96밀리초가 걸렸고, 그 사이에 친 글자는 팔레트가 아니라 직전에 입력을 받던 곳으로 갔습니다. 빠르게 치면 첫 글자가 들어가고도 남는 시간이라 실제로 사라졌습니다. 이제 그 사이에 도착한 입력을 모아 두었다가 팔레트가 키보드를 넘겨받는 순간 검색어에 넣습니다. (`Sources/AppDelegate.swift`, `Sources/ContentView.swift`)

- **방향키와 Ctrl+N·P·J·K가 이따금 선택을 옮기지 않던 문제** — 같은 원인입니다. 팔레트의 이동 키는 검색 입력란이 키보드를 가진 뒤에만 동작하는데, 그 전에 누른 키는 어디에도 도달하지 않았습니다. (`Sources/ContentView.swift`)

- **한글·일본어·중국어를 입력하는 중에 팔레트가 뜨면 조합이 깨지던 문제** — 조합 중인 키는 입력기 몫인데 팔레트가 가로채, 조합이 이어지지도 완성되지도 않은 채 낱자가 검색어에 들어갔습니다. (`Sources/AppDelegate.swift`)

- **이름 바꾸기 중에 친 글자가 나중에 검색어에 나타나던 문제** — 이름 바꾸기 칸이 뜬 직후 친 글자가 그 칸에 들어가지 않고 사라졌다가, 팔레트가 검색 모드로 돌아올 때 검색어에 붙었습니다. (`Sources/ContentView.swift`)

- **원격 판이 닫힐 때 마지막 입력이 사라지던 문제** — 상대가 연결을 닫았는데 이미 보낸 입력이 아직 남아 있으면, 그것을 읽지 않고 종료했습니다. (`daemon/term-mesh-peer-relay/src/main.rs`)

## [0.216.0] - 2026-08-29

**사이드바에서 사라진 것처럼 보이던 피어의 프로젝트를 호스트 목록에도 표시하고, 데몬이 종료될 때 릴레이가 끊겨 있는 시간을 줄였습니다.**

### Fixed

- **호스트별로 묶으면 보이지 않던 피어의 프로젝트** — Mac 피어는 워크스페이스를 앱 소켓에서 제공하지만 프로젝트 목록은 세션을 소유한 데몬만 발행합니다. 그래서 사이드바를 호스트별로 묶어 보면 그 머신의 워크스페이스만 나오고, 거기서 실행 중인 프로젝트는 어디에도 표시되지 않았습니다. 지워진 것처럼 보였지만 실제로는 프로젝트별 보기에 그대로 있었고, 그쪽을 가리키는 안내도 없었습니다. 이제 두 보기가 같은 기준으로 같은 목록을 보여줍니다. (`Sources/SidebarViews.swift`)

- **복원 중에 목록에서 빠지던 원격 프로젝트** — 복원이 진행되는 동안 상태가 유지되지 않아 행이 잠시 사라졌습니다. (`Sources/SidebarViews.swift`, `Sources/TeamOrchestrator+RemoteAgent.swift`)

- **묶는 기준을 알 수 없던 사이드바 설명** — "호스트별·프로젝트별로 묶기"는 같은 행을 다시 늘어놓는다는 뜻으로 읽혔고, 다른 경로로 넘긴 Mac의 프로젝트가 어느 목록에 나오는지는 설명이 없었습니다. 각 보기가 무엇을 기준으로 삼는지와 그 프로젝트가 어디에 표시되는지를 문구에 담았습니다. 동작은 그대로입니다. (`Sources/SidebarViews.swift`, `Sources/PeerHostEditorView.swift`)

- **데몬이 강제 종료될 때까지 멈춰 있던 종료 과정** — term-meshd가 종료 도중 진행을 멈춰, 최근 재시작 네 번 모두 시스템의 강제 종료를 기다렸습니다. 어느 단계에서 멈췄는지도 기록에 남지 않았습니다. 이제 각 단계에 이름과 제한 시간을 두고 별도 감시 스레드가 진행 상황을 지켜봅니다. 한 단계가 막혀도 제한 시간 안에 그 단계를 기록하고 종료하므로, 릴레이가 끊겨 있는 시간이 종료 제한시간 전체에서 몇 초로 줄어듭니다. (`daemon/term-meshd/src/shutdown.rs`)

- **재사용된 PID에 신호를 보내던 프로세스 감시** — 감시하던 프로세스가 끝나고 다른 프로세스가 같은 PID를 물려받으면 그 프로세스를 멈출 수 있었습니다. 또 한 번 감시에서 빠진 프로세스는 다시 등록되지 않아 남은 수명 내내 감시 밖에 있었고, 대시보드는 3초마다 등록과 해제를 반복하면서 실제로는 아무것도 감시하지 못했습니다. 이제 데몬의 목록을 기준으로 매번 대조하고, 신호를 보내기 직전에 대상이 같은 프로세스인지 다시 확인합니다. (`daemon/term-meshd/src/monitor.rs`)

- **진행 중인 턴에 프롬프트가 흡수되면 뒤집히던 리더 턴 기록** — 종료 신호가 가장 최근 항목을 닫도록 되어 있어, 흡수된 프롬프트가 닫히고 실제로 경로를 보고한 턴은 끝나지 않은 채 남았습니다. 그 결과 경로를 보고한 턴이 미완료로, 아무것도 보고하지 않은 턴이 정상으로 집계됐습니다. 이제 경로를 보고한 턴을 닫습니다. 또한 리더 판이 다른 호스트에서 돌면 기록이 그 호스트에 남는다는 사실을 함께 표시해, "여기서 읽을 수 없음"과 "보고된 턴이 없음"을 구분할 수 있습니다. (`Sources/LeaderTurnLog.swift`, `scripts/leader-turn-hook.sh`)

- **원격 판의 릴레이가 시작되지 않던 경우** — 양쪽 연결 중 나중에 완성되는 쪽을 기준으로 릴레이를 시작하도록 고쳤습니다. (`Sources/GhosttyTerminalView.swift`, `Sources/PeerPaneSession.swift`)

- **설정 화면에 영어로 남아 있던 셸 상태 요약과 라벨** — (`Sources/SettingsView.swift`, `Resources/Localizable.xcstrings`)

### Security

- **팀 데이터 명령의 권한 확인 보강** — 권한 검사를 거치지 않는 작업 지표 읽기 경로가 중복으로 남아 있어 제거했고, 원격에서 호출 가능한 메서드 목록이 실제 처리기와 일치하는지 확인하도록 했습니다. 목록에는 있지만 처리기가 없는 메서드가 검사를 통과한 뒤 상대편에서 실패하던 문제를 막습니다. (`Sources/TerminalController.swift`)
...
- **앱을 다시 켤 때마다 같은 Project workspace가 하나씩 더 생기던 문제** ([#326](https://github.com/x-mesh/term-mesh/pull/326)) — `session.json`이 workspace를 복원하면서 그 정체성(ID)까지는 저장하지 않아, 복원된 workspace는 매번 새 ID를 받았다. Project 선언은 그 ID로 보관돼 있어서 재시작 후에는 조회가 항상 실패했고, 그때마다 term-mesh는 "이 Project를 담은 workspace가 아직 없다"고 판단해 `[project]` workspace를 새로 만들었다. team 3개를 쓰는 호스트에서 재시작 한 번에 3개씩 늘어났다. 이제 workspace ID를 세션에 함께 저장하고, ID가 없는 기존 세션은 workspace 제목으로 Project를 알아본다. workspace를 닫으면 그 선언도 함께 지운다. — thanks @JINWOO-J!
...
- **Project에 필요할 때 한 번씩 Pair Review를 돌릴 수 있다** ([#275](https://github.com/x-mesh/term-mesh/pull/275)) — 사이드바에서 Project를 우클릭하면 "Pair Review…"가 생겼다. 지금까지 리뷰를 받으려면 Watch를 계속 켜두거나 리더에게 직접 부탁하는 수밖에 없었다. 이제 시트에서 범위(현재 작업 / 현재 변경분 / 브랜치 diff), 관점(General·Security·Tests·직접 입력), 리뷰어 모델을 고르고 한 번만 실행하면 된다. 실행하는 순간 대상이 그대로 고정되므로 리뷰가 도는 동안 코드가 바뀌어도 결과가 흔들리지 않고, 리뷰어는 읽기 전용이라 파일을 고치거나 커밋하지 않는다. 켜져 있는 Watch 설정도 건드리지 않는다. 판정과 소요 시간은 시트에 바로 표시되고 리더에게도 전달되며, 시작 응답이 늦어 다시 눌러도 같은 리뷰에 재연결되어 두 번 돌지 않는다.

### Changed

- **리뷰만 요청했을 때 결과가 더 빨리 나온다** — 지금까지 리더는 리뷰 요청도 구현 요청과 똑같이 처리해서, 리더가 전체 리뷰를 끝낸 뒤에야 검증 담당을 붙였다. 이제 이미 존재하는 diff에 대한 리뷰 전용 요청이면 리더는 대상만 고정한 뒤 곧바로 읽기 전용 검증자를 최대 2명 붙이고, 그동안 자신은 전체 흐름 점검과 독립 검증을 병행한다. 범위가 좁은 리뷰는 지금처럼 리더가 직접 처리한다.
...
- **split divider 색을 직접 고를 수 있다** — Settings의 Terminal 항목에 "Split Divider Color"가 생겼다. 지금까지 pane을 나누는 경계선 색은 ghostty 설정값에 고정돼 있었고 앱에서는 바꿀 수 없었다. 이제 색을 지정하면 그 값이 터미널 설정에 반영되고, "Reset"을 누르면 다시 ghostty config 값을 따른다.

### Changed

- **Project 멀티 에이전트가 무조건 병렬로 흩뿌리지 않는다** ([#272](https://github.com/x-mesh/term-mesh/pull/272)) — 지금까지 Project 리더는 작업을 기본적으로 여러 worker에게 나눠 던졌다. 작은 작업이나 순서를 지켜야 하는 작업까지 쪼개다 보니, 조율에 드는 시간이 실제 작업보다 커지는 경우가 있었다. 이제 리더는 요청을 direct(직접 처리) / probe(읽기 전용 확인 1건) / parallel(2~3건 분배)로 분류하고, 서로 건드리는 파일이 겹치지 않으면서 각각 독립적으로 검증할 수 있는 작업이 둘 이상일 때만 worker를 띄운다. 분배한 뒤에도 리더는 worker가 소유한 파일을 건드리지 않고 통합·수용 검증만 맡는다.

### Fixed

- **리더에게 보낸 긴 요청이 잘리거나 사라질 수 있던 문제** — 사용자 요청은 그동안 터미널 입력으로 전달돼서, 리더가 아직 준비되지 않았거나 입력이 중간에 끊기면 복구할 방법이 없었다. 이제 요청 본문은 board에 먼저 영속 저장되고 터미널에는 짧은 wake-up만 보낸다. 리더는 준비된 뒤 저장된 본문을 정확히 한 번 가져가며, 내용이 온전한지 확인한 뒤에만 처리한다. 요청 본문을 읽을 권한도 해당 Project 리더에게만 주어진다.

- **같은 역할의 에이전트가 둘 이상일 때 엉뚱한 쪽으로 명령이 가던 문제** — 이름만으로 대상을 고르면 중복된 역할 중 어느 쪽인지 확정할 수 없었다. 이제 `--agent-instance-id`로 정확한 인스턴스를 지정할 수 있고, 모호한 이름 지정은 조용히 아무 쪽이나 고르는 대신 거부된다. 이름이 유일한 경우는 지금처럼 그대로 동작한다.
...
- **CLI 도구가 보내는 데스크톱 알림이 오지 않던 문제** — Claude Code처럼 "다 끝났다"를 터미널로 알리는 도구들은 Kitty의 OSC 99 방식을 쓴다. 그런데 터미널이 그 코드를 **아예 알아보지 못하고 있었다** — `99`를 읽어야 할 자리가 없어서, 보낸 알림은 그대로 버려졌다. 백그라운드 탭에서 작업이 끝나도 알림이 안 오던 이유가 이것이다. 이제 제목과 본문이 나뉘어 오는 긴 알림까지 이어 붙여서 표시한다.

- **원격 native 에이전트가 셸 설정을 못 읽던 문제** — 다른 기기의 에이전트는 그 계정의 로그인 셸로 시작한다. 그런데 데몬이 systemd에서 물려받은 `SHELL=/bin/sh`가 계정의 실제 셸(`chsh`로 zsh를 쓰는 계정이라도)을 덮어써서, `~/.zshenv` 같은 파일을 건너뛰었다. **SSH로 접속하면 보이던 API 키가 에이전트에게만 안 보이는** 상태였고, 그래서 준비 점검은 통과하는데 에이전트만 시작하지 못했다. 이제 `/etc/passwd`의 계정 셸을 먼저 따른다.

  그리고 그 값을 **눈으로 확인할 수 있게 했다.** 프로젝트 만들기 화면과 Settings → Peer Hosts의 호스트 점검이 이제 그 기기가 쓸 셸과, 셸과 무관하게 읽히는 `~/.config/term-mesh/agent-env` 파일의 경로·존재 여부를 함께 보여준다. 값 자체는 읽지 않는다 — 있는지 없는지와 어디에 두면 되는지만 말한다.

- **원격 에이전트 턴이 실패했는데 이유가 안 보이던 문제** — 턴이 깨지면 빨간 `failed` 표시만 남고, 실제 원인(제공자 오류 메시지 같은 것)은 화면 어디에도 나타나지 않았다. 그 내용이 답변 스트림이 아니라 종료 이벤트에만 실려 오는데, 그 자리를 아무도 그리지 않았기 때문이다. 이제 **이미 화면에 나온 내용과 겹치지 않는 부분만 골라** 실패 사유로 붙는다. 자격 증명처럼 보이는 값은 가려진다.

- **에이전트·프로젝트 실패가 Activity에 남지 않던 문제** — pane을 닫으면 실패의 흔적도 함께 사라졌다. 이제 실패한 턴과 **만들지 못한 프로젝트**가 Live Activity 목록에 기록되고, 줄마다 정보·경고·오류가 구분돼서 오류는 로그 수준과 상관없이 항상 보인다.

- **원격 프로젝트를 지울 때 삭제가 중간에 실패하던 문제** — 프로젝트 전용 workspace에 pane이 하나만 남으면, 앱은 그 마지막 pane을 먼저 지우려 했다. 그런데 "마지막 pane은 지우지 않는다"는 규칙에 막혀서, **성공했어야 할 삭제 전체가 실패로 끝났다.** 이제 workspace를 통째로 지우는 쪽에 맡긴다 — 그 안의 pane은 함께 사라지고, 그 바깥에 있는 에이전트 surface만 따로 정리한다.

  판단 근거도 호스트에게 직접 묻는다. 데몬을 올린 뒤에는 예전 방식으로 만든 pane과 새 전용 workspace가 **한 기기에 섞여 있을 수 있어서**, "이 호스트에 전용 workspace가 있다"만으로는 개별 pane의 소속을 알 수 없다. 지금은 그 workspace의 실제 구성을 받아 확인하고, 확인에 실패하면 지우지 않고 그대로 남긴다 — 다시 시도할 수 있도록.

- **CLI로 만든 workspace의 첫 명령이 실행되지 않던 문제** — 화면을 뺏지 않고 workspace를 만들면서 명령을 같이 넘기면(`--no-focus`), 그 workspace는 화면에 올라오지 않아 터미널이 실체화되지 않았고 **명령은 그냥 사라졌다.** 이제 그 명령이 돌 터미널이 준비될 때까지 붙잡아 둔다.

- **아직 시작 중인 pane에 CLI로 키를 보내면 버려지던 문제** — `Ctrl-C`나 Enter를 보냈는데 그 pane의 터미널이 아직 안 올라왔으면 `Surface not ready`로 끝났다. 스크립트 입장에선 성공도 실패도 아닌 상태였다. 이제 **입력을 큐에 담아 두고** 터미널이 뜨는 대로 흘려보낸다. 응답에 `queued` 여부가 함께 온다.
...
**다른 기기에 맡긴 일이 내 앱보다 오래 산다.** 원격 에이전트를 그쪽 호스트가 직접 소유하게 되면서, 이 앱을 껐다 켜도 저쪽 작업은 그대로 이어진다. 그리고 원격 리더가 지시문 없이 뜨거나 아무 말 없이 멈추던 문제들을 걷어냈다 — 실패해도 이제는 이유가 남는다.

### Added

- **원격 에이전트를 그 기기가 소유한다** — 지금까지 다른 기기의 native 에이전트는 이 앱이 붙어 있는 동안만 존재했다. 이제 그 호스트가 에이전트의 수명을 갖고 있어서, **이쪽 앱을 종료했다 다시 열어도 저쪽 작업은 끊기지 않는다.** 에이전트에 넘기는 환경 변수(API 키 같은 것)도 **ssh 명령줄에 실리지 않고** 파일로 전달된 뒤 실행 직전에 지워진다 — 명령줄은 그 기기의 다른 프로세스에서도 보이기 때문이다. 이 방식을 지원하지 않는 구버전 호스트에는 그 사실을 알려주고, 팀 메시지를 주고받을 수 없는 호스트라면 팀을 만들기 전에 막는다. 호스트 목록에는 그 기기가 실제로 돌리는 버전과 지원 기능이 함께 보인다.

- **브랜치 칸에서 Tab이 이름을 완성한다** — 프로젝트를 만들 때 브랜치 이름을 끝까지 타이핑해야 했다. 이제 **Tab**을 누르면 셸처럼 완성된다. 후보가 여럿이면 **모두가 일치하는 데까지만** 채운다 — `fea` → `feature/`. 하나로 좁혀지면 끝까지 채운다. 대소문자는 무시하지만 채워지는 이름은 저장소의 표기를 따르므로, 완성된 이름은 그대로 git이 받는 이름이다. 완성할 게 없으면 Tab은 원래대로 다음 칸으로 넘어간다.

- **없는 브랜치를 적었는지 그 자리에서 알려준다** — 지금까지는 있는 브랜치와 없는 브랜치가 똑같이 보였고, 오타는 **몇 초 뒤 `git clone` 안에서** 창이 닫힌 다음에야 드러났다. 이제 저장소에 있는 이름이면 **초록 체크**로, 없는 이름이면 **주황 경고와 기본 브랜치 안내**로 바로 구분된다. 비워 두면 기본 브랜치를 쓰므로 아무 표시도 하지 않는다.
...
- **다른 기기에 리더를 만들 때 간헐적으로 멈추던 문제** — 리더는 저쪽 기기에 pane을 하나 더 열어 달라고 요청하고 그게 나타나길 기다린다. 그런데 새로 만든 pane이 **화면에 올라와 있지 않은 workspace**에 생기면 터미널이 실체화되지 않고, 그러면 목록에도 안 잡혀서 요청한 쪽은 10초를 기다리다 포기했다. 기존에 그 workspace를 붙잡아 두는 장치가 있었지만 **첫 pane 하나가 준비되면 바로 손을 놓아서**, 그 뒤에 만들어지는 pane은 보호를 받지 못했다. 타이밍에 따라 되기도 하고 안 되기도 해서 원인을 짚기 어려웠다. 이제 **요청한 그 pane이 준비될 때까지** 붙잡아 둔다. 그리고 실패했을 때 양쪽 모두 이유를 기록한다 — 지금까지는 요청이 조용히 사라져 아무 흔적도 남지 않았다.

- **원격 리더가 지시문 없이 뜨거나 아예 시작하지 못하던 문제** — 리더에게 줄 지시문을 **pane에 타이핑해서** 넘기고 있었다. 7KB짜리 지시문은 base64로 10KB, 명령 14개가 되는데 **터미널 입력 버퍼는 1KB다.** 대부분이 조용히 사라졌고, 앱은 전송이 다 성공했다고 믿었다. 그래서 리더가 빈 지시문으로 시작하거나 화면이 멈춘 채 끝났다. 이제 **SSH로 한 번에 쓰고**, 그 파일이 pane에서 보이는지 pane에게 직접 확인한 뒤, 보이지 않는 호스트에서만(systemd `PrivateTmp` 같은 경우) 타이핑 방식으로 넘어간다. 그 경우에도 명령 사이 간격을 넉넉히 둔다.

- **프로젝트 생성 창에서 "기기가 준비되지 않았다"고만 하고 끝나던 문제** — 원격 기기가 연결되지 않으면 만들기 버튼이 꺼진 채 경고만 떴다. **그 창 안에는 연결할 방법이 없어서**, 사이드바로 나갔다 오면 입력하던 내용이 사라졌다. 이제 그 문장 옆에 **연결/다시 시도 버튼**이 함께 뜬다. 문구도 상황을 구분해서 말한다 — 아직 연결 안 됨, 연결 실패(이유 포함), 연결은 됐지만 도구 위치를 확인하는 중.

- **노트북을 덮어두면 원격 리더가 팀을 잃던 문제** ([#143](https://github.com/x-mesh/term-mesh/issues/143)) — 원격 리더의 권한은 30분마다 갱신되는데, 그 타이머는 **맥이 자는 동안 흐르지 않는다.** 뚜껑을 남은 유효시간보다 오래 닫아두면 깨어났을 때 이미 만료라, 그 pane에서 `tm-agent`가 전부 실패했다. 노트북에선 드문 일도 아니다. 이제 **깨어나는 즉시 갱신**한다. 깨어난 직후엔 네트워크가 아직 안 붙어 있을 수 있어서, 이때 실패해도 팀을 정리하지 않고 다음 정규 갱신에 맡긴다 — 멀쩡한 팀을 네트워크가 늦었다는 이유로 잃지 않도록.
