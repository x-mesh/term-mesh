...
- **프로젝트를 만들 때 원격 폴더를 눈으로 보고 고른다** — 피어 호스트에 프로젝트를 만들려면 그쪽 경로를 외워서 타이핑해야 했고, 틀렸다는 사실은 만들고 나서야 알았다. 이제 경로 칸 옆의 **Browse…** 가 그 호스트의 폴더 목록을 열고, 타이핑하는 동안에도 후보 폴더가 아래에 뜬다. 상위 폴더로 올라가기, 다시 읽기, "여기에 만들기"가 그 자리에서 된다.

- **에이전트 턴이 언제 끝났는지 보인다** — 끝난 턴은 지금까지 "얼마나 걸렸는지"만 말했다. 여러 pane이 아무도 안 보는 사이에 끝나 있으면 **어느 게 먼저 끝났는지, 지금 보는 것이 한 시간 전에 끝난 것인지** 알 방법이 없었다. 이제 끝난 시각이 맨 앞에 온다 — `17:41:06 (2m 14s)`. 24시간제로 고정해서 좁은 pane에서도 폭이 늘지 않고, 옆 pane과 나란히 하나의 시간표로 읽힌다. CLI가 소요 시간을 안 알려주는 경우에도 이제 앱이 직접 재서 채운다.

- **피어 호스트에 지정한 PATH가 무시되지 않고 더해진다** — Edit Peer Host에 PATH를 저장해도 통째로 버려졌다. 그런데 준비 검사는 로그인 셸의 PATH까지 훑고 실행은 고정된 여덟 곳만 뒤지기 때문에, `~/.npm-global/bin`이나 `~/bin`, pyenv shim에 있는 CLI는 **검사에서는 있다고 나오고 실행은 못 찾는다** — pane은 열리는데 에이전트가 영영 시작하지 않는 그 화면이다. 이제 저장한 경로가 기본 경로 **뒤에** 붙는다. 뒤인 것이 핵심이다 — 앞에 오면 호스트의 같은 이름 파일이 `/usr/bin`이나 term-mesh가 깔아 둔 CLI를 가릴 수 있다. 특정 실행 파일을 지정하고 싶으면 그 CLI의 절대 경로를 설정하면 된다.

- **아무 말 없이 멈춘 pane이 이유를 말한다** — 실패는 늘 보고된다: 종료 코드, 시그널, 끊긴 연결. 보고할 사람이 없는 경우는 하나뿐이다 — **실패하지도, 시작하지도 않은 실행.** 이제 30초 동안 아무것도 도착하지 않으면 pane이 짐작되는 원인을 적는다. 원격 에이전트라면 그 호스트의 실행 PATH에 CLI가 없을 수 있다는 것과 어디서 고치는지까지 함께 알려준다.

### Fixed

- **"Disconnect" 하나가 서로 다른 두 가지 일을 부르던 문제** — 사이드바에서 호스트 보기를 그만두는 것과 그 호스트와의 연결을 끊는 것이 같은 단어를 쓰고 있었고, 저쪽 기기의 프로세스가 같이 죽는지 여부는 어디에도 적혀 있지 않았다. 이제 둘은 다른 이름의 다른 동작이다. **Stop Browsing** 은 사이드바에서만 손을 떼고 pane은 전부 그대로 둔다. **Disconnect Host** 는 이쪽 화면(pane·미러·릴레이)을 닫고 연결을 접는다. **어느 쪽도 원격 기기의 프로세스를 끝내지 않는다.** pane은 자리를 지키며 재연결을 제안하고, 다시 연결하면 낡은 통로를 되살리는 대신 새 연결을 연다.

- **원격 리더가 시스템 프롬프트 없이 뜨던 문제** — 프롬프트 파일은 SSH 점검 과정이 쓰고 pane이 읽었는데, 이 둘이 늘 같은 파일 시스템인 것은 아니다. `PrivateTmp`를 쓰는 시스템 서비스나 컨테이너에서는 **SSH 쪽에는 있고 pane 쪽에는 없는** 파일이 되고, 리더는 프롬프트 없이 시작했다. 이제 CLI를 실행할 그 셸이 프롬프트를 직접 만들고, 길이가 맞지 않으면 아예 실행하지 않는다.

- **원격 pane에 붙여넣은 파일을 에이전트가 못 찾던 문제** — 붙여넣은 파일은 `/tmp/term-mesh-paste`로 갔는데, 이 경로가 하나의 폴더인 것은 양쪽이 `/tmp`를 공유할 때뿐이다. `PrivateTmp=true`에서는 두 개다 — SSH가 만든 파일을 pane은 자기 `/tmp`에서 찾았고, 에이전트는 없는 경로를 받았다. 이제 접속 계정의 캐시 폴더를 쓴다. 설치 스크립트도 접속 계정을 그대로 따라가도록 맞췄다 — `sudo` 설치는 `SUDO_USER`, root 직접 접속은 root이며, `TERMMESH_SERVICE_USER`로 지정하면 그 값이 우선한다.
