...
def _focused_surface_id(client: termmesh) -> str:
...
def main() -> int:
    token = "TERMMESH_PALETTE_FOCUS_PROBE_9412"
    restore_token = "TERMMESH_PALETTE_RESTORE_PROBE_7731"
    ...
    with termmesh(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise termmeshError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise termmeshError("terminal did not receive typing after closing command palette")
