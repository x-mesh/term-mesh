...
def main():
    with termmesh(SOCKET_PATH) as client:
        ...
        if not after_rows:
            raise termmeshError(f"no results for 'rename' after replacement: {after}")
        top_after = str(after_rows[0].get("command_id") or "")
        if top_after not in {"palette.renameWorkspace", "palette.renameTab"}:
            raise termmeshError(f"top result did not update to rename command after replacement: {after_rows[0]}")
