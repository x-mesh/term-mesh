...
def _grid_metrics_are_valid(c, sid) -> bool:
    grid = c.read_grid(sid)
    return grid.get("padding_metrics_valid") is True
...
def main() -> int:
    ...
    with termmesh() as c:
        ...
        try:
            ...
            if not _wait(lambda: _grid_metrics_are_valid(c, sid), timeout_s=8):
                grid = c.read_grid(sid)
                raise termmeshError(
                    "Ghostty grid metrics were unavailable after peer "
                    f"Resize(41,13): grid={grid!r}"
                )
            ...
            if not _wait(lambda: _grid_metrics_are_valid(c, sid), timeout_s=8):
                ...
                raise termmeshError(
                    "Ghostty grid metrics were unavailable after peer "
                    f"Resize(40,12): grid={grid!r}"
                )
            grid = c.read_grid(sid)
            padding_left = float(grid.get("padding_left") or 0)
            padding_top = float(grid.get("padding_top") or 0)
            if padding_left <= 0 and padding_top <= 0:
                print(
                    "SKIP: Ghostty grid metrics report zero padding; "
                    "the resize regression is not exercised "
                    f"(padding_left={padding_left}, padding_top={padding_top})"
                )
                return 0
            ...
            if peer is not None:
                if attached and sid_bytes is not None:
                    try:
                        peer.detach_surface(sid_bytes)
                    except (OSError, pc.PeerClientError):
                        pass
                peer.close()
            if sid is not None:
                try:
                    ...
                    pass
            for path in (script, repaint_trigger):
                try:
                    path.unlink()
                except OSError:
                    pass
