...
def _run_cli_expect_failure(cli: str, args: list[str], needles: list[str]) -> None:
    ...
    if proc.returncode == 0:
        raise termmeshError(f"Expected CLI failure for {' '.join(args)}, but it succeeded: {proc.stdout}")
    merged = f"{proc.stdout}\n{proc.stderr}"
    if not any(needle in merged for needle in needles):
        raise termmeshError(f"Expected CLI failure containing one of {needles!r} for {' '.join(args)}, got: {merged}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="termmesh-browser-cli-") as root:
        root_path = Path(root)
...
def main() -> int:
    ...
    with _local_test_server() as page_url:
        ...
        state_file = tempfile.NamedTemporaryFile(delete=False, prefix="termmesh-cli-state-", suffix=".json").name
