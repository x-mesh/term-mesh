...
def _wait_for_terminal_focus(c: termmesh, panel_id: str, timeout_s: float = 4.0) -> None:
...
def _baseline_all(c: termmesh, panel_ids: list[str], label: str) -> None:
    for pid in panel_ids:
        c.panel_snapshot(pid, label=f"{label}_base_{pid[:6]}")


def _after_all(c: termmesh, panel_ids: list[str], label: str) -> dict[str, int]:
    diffs: dict[str, int] = {}
    for pid in panel_ids:
...
def _assert_routing(diffs: dict[str, int], target: str, *, min_changed: int = 250, ratio: float = 3.0) -> None:
    ...
    if tgt is None:
        raise termmeshError(f"missing diff for target {target}")
    # -1 means first diff or size mismatch; treat as failure here.
    if tgt < min_changed:
        raise termmeshError(f"target panel did not change enough (changed_pixels={tgt}): diffs={diffs}")
...
def main() -> int:
    with termmesh(SOCKET_PATH) as c:
        ...
        for pid in panel_ids:
            c.send_surface(pid, f"echo TERMMESH_READY_{pid[:6]}\n")
        time.sleep(0.6)
        ...
        for i, target in enumerate(panel_ids):
            marker = f"TERMMESH_ROUTE_{i}_{target[:6]}"
            ...
            if marker not in text:
                raise termmeshError(f"marker missing from read_terminal_text for {target}: {marker}")
