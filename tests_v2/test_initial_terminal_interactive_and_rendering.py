...
def _wait_for_surface_focus(c: termmesh, panel_id: str, timeout_s: float = 5.0) -> None:
    ...
    raise termmeshError(f"Timed out waiting for surface focus: {panel_id}")


def _wait_for_render_context(c: termmesh, panel_id: str, timeout_s: float = 5.0) -> dict:
    ...
    while time.time() - start < timeout_s:
        ...
        if bool(last.get("inWindow")):
            return last
        time.sleep(0.1)
    raise termmeshError(f"Expected inWindow render context, got: {last}")


def main() -> int:
    token = f"TERMMESH_INIT_{int(time.time() * 1000)}"
    tmp = f"/tmp/term-mesh_init_{token}.txt"
    with termmesh(SOCKET_PATH) as c:
        ...
        if not surfaces:
            raise termmeshError("Expected at least 1 surface after new_workspace")
