#!/usr/bin/env python3
"""
Visual Screenshot Tests for termmesh
...
def get_client() -> termmesh:
    c = termmesh(SOCKET_PATH)
    c.connect()
    return c


def take_screenshot(client: termmesh, label: str) -> Optional[Screenshot]:
...
def capture_state(client: termmesh) -> str:
    try:
...
def stamp_terminals(client: termmesh, label: str) -> None:
    ...
    for h in terminal_surfaces:
        ...
        try:
            ...
            client.send_surface(idx, f"echo TERMMESH_VIS {safe} surf={idx}\n")
        except Exception:
            pass


def capture(client: termmesh, label: str):
...
def reset_workspace(client: termmesh) -> termmesh:
...
def surface_count(client: termmesh) -> int:
    return len(client.list_surfaces())

def pane_count(client: termmesh) -> int:
...
def wait_surface_count(client: termmesh, expected: int, timeout: float = 3.0) -> bool:
    start = time.time()
...
def wait_url_contains(client: termmesh, panel_id: str, needle: str, timeout: float = 8.0) -> bool:
...
def cleanup_workspaces(client: termmesh):
...
def _verify_surface_responsive(client: termmesh, surface_idx: int, marker: Path,
                                retries: int = 3) -> bool:
...
def verify_views_in_window(client: termmesh, label: str = "", timeout: float = 5.0) -> Optional[str]:
...
def verify_all_responsive(client: termmesh, label: str = "") -> Optional[str]:
    ...
    for idx, h in enumerate(terminal_surfaces):
        surface_idx = h["index"]
        marker = Path(tempfile.gettempdir()) / f"termmesh_vis_{os.getpid()}_{idx}"
        try:
...
def test_a1_initial_state(client: termmesh) -> StateChange:
...
def test_a2_split_right(client: termmesh) -> StateChange:
...
def test_a3_split_down(client: termmesh) -> StateChange:
...
def _close_and_verify(client: termmesh, change: StateChange, close_idx: int,
...
def test_b4_close_right(client: termmesh) -> StateChange:
...
def test_b5_close_left(client: termmesh) -> StateChange:
...
def test_b6_close_bottom(client: termmesh) -> StateChange:
...
def test_b7_close_top(client: termmesh) -> StateChange:
...
def test_c8_3way_close_middle(client: termmesh) -> StateChange:
...
def test_c9_grid_close_topleft(client: termmesh) -> StateChange:
...
def test_c10_grid_close_bottomright(client: termmesh) -> StateChange:
...
def test_d11_nested_close_bottomright(client: termmesh) -> StateChange:
...
def test_d12_nested_close_top(client: termmesh) -> StateChange:
...
def test_d13_4pane_close_second(client: termmesh) -> StateChange:
...
def test_e14_browser_close_terminal(client: termmesh) -> StateChange:
...
def test_e15_browser_close_browser(client: termmesh) -> StateChange:
...
def test_f16_nested_tabs_close_first(client: termmesh) -> StateChange:
...
def test_g17_rapid_down_close_top(client: termmesh) -> StateChange:
...
def test_g18_rapid_right_close_left(client: termmesh) -> StateChange:
...
def test_g19_alternating_close_reverse(client: termmesh) -> StateChange:
...
def test_h20_workspace_switch_back(client: termmesh) -> StateChange:
...
def _create_browser_surface(client: termmesh, url: Optional[str] = None) -> str:
    return client.new_surface(panel_type="browser", url=url)


def test_i21_browser_drag_split_right_wait_load(client: termmesh) -> StateChange:
...
def test_i22_browser_drag_split_right_immediate(client: termmesh) -> StateChange:
...
def test_i23_browser_drag_split_right_webview_focused(client: termmesh) -> StateChange:
...
def test_i24_browser_drag_split_right_focus_bounce(client: termmesh) -> StateChange:
...
def test_i25_browser_drag_split_right_then_switch_panes(client: termmesh) -> StateChange:
...
def test_i26_browser_drag_split_right_initial_url(client: termmesh) -> StateChange:
...
def test_i27_browser_drag_split_right_after_reload(client: termmesh) -> StateChange:
...
def test_i28_browser_drag_split_right_double_drag(client: termmesh) -> StateChange:
...
<html>
<head>
    <title>termmesh Visual Test Report</title>
    <style>
        body {
...
<body>
    <h1>termmesh Visual Test Report</h1>
...
def run_visual_tests():
    ...
    print("=" * 60)
    print(f"termmesh Visual Screenshot Tests ({len(test_fns)} scenarios)")
    print("=" * 60)
    print()
