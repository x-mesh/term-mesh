...
def _rename_surface(client: termmesh, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: termmesh, workspace_id: str) -> str:
...
def main() -> int:
    with termmesh(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise termmeshError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise termmeshError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise termmeshError("switcher returned no rows for renamed surface query")
