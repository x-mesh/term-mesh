...
<!doctype html>
<html>
  <head><title>termmesh-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("termmesh" in str(out.get("value", "")), f"Expected #out text to include 'termmesh': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "termmesh", f"Expected eval value 'termmesh': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("termmesh-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on termmesh webview with ref handles")
    return 0
