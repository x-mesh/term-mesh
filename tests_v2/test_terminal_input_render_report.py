...
SOCKET_PATH = os.environ.get("TERMMESH_SOCKET") or os.environ.get("TERMMESH_SOCKET_PATH") or "/tmp/term-mesh-debug.sock"
...
def _focused_panel_id(c: termmesh) -> str:
    surfaces = c.list_surfaces()
    if not surfaces:
        raise termmeshError("Expected at least 1 surface")
    return next((sid for _i, sid, focused in surfaces if focused), surfaces[0][1])


def _snap_panel(c: termmesh, panel_id: str, label: str) -> Shot:
...
def _panel_sequence_blink_and_type(c: termmesh, panel_id: str, prefix: str, typed_char: str = "x") -> tuple[list[Shot], dict]:
    shots: list[Shot] = []
...
<head>
  <meta charset="utf-8" />
  <title>termmesh terminal input render report</title>
  <style>
    :root {{
      --bg: #0b0f14;
      --panel: #111826;
...
<body>
  <h1>termmesh terminal input render report</h1>
