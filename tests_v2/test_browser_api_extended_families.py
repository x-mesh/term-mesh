...
def _expect_error_contains(label: str, fn, needle: str) -> None:
    try:
        ...
        if needle in text:
            return
        raise termmeshError(f"{label}: expected error containing {needle!r}, got: {text}")
    raise termmeshError(f"{label}: expected error containing {needle!r}, but call succeeded")


def _wait_selector(c: termmesh, surface_id: str, selector: str, timeout_s: float = 6.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise termmeshError(f"Timed out waiting for selector {selector}")


def _wait_function(c: termmesh, surface_id: str, expression: str, timeout_s: float = 6.0) -> None:
    timeout_ms = max(1, int(timeout_s * 1000.0))
    try:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise termmeshError(f"Timed out waiting for function: {expression}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="termmesh-browser-ext-") as root:
        root_path = Path(root)
...
<html>
  <head>
    <title>termmesh-browser-extended-second</title>
  </head>
  <body>
...
<html>
  <head>
    <title>termmesh-browser-extended</title>
    <style>
  ...
  <body>
    ...
    <script>
      ...
      window.emitConsoleAndError = function () {
        console.log('termmesh-console-entry');
        setTimeout(function () {
          throw new Error('termmesh-boom');
        }, 0);
        return true;
      };
    </script>
...
def main() -> int:
    with _local_test_server() as base_url:
        ...
        with termmesh(SOCKET_PATH) as c:
            ...
            download_path = tempfile.NamedTemporaryFile(delete=False, prefix="termmesh-download-", suffix=".txt").name
            ...
            c._call(
                ...
                {
                    "surface_id": sid,
                    "name": "termmesh_cookie",
                    "value": "cookie_value",
                    "url": index_url,
                },
            )
            got_cookie = c._call("browser.cookies.get", {"surface_id": sid, "name": "termmesh_cookie"}) or {}
            cookies = got_cookie.get("cookies") or []
            _must(any(str(row.get("name")) == "termmesh_cookie" for row in cookies), f"Expected termmesh_cookie in cookies.get: {got_cookie}")
            c._call("browser.cookies.clear", {"surface_id": sid, "name": "termmesh_cookie"})
            got_after_clear = c._call("browser.cookies.get", {"surface_id": sid, "name": "termmesh_cookie"}) or {}
            ...
            c._call("browser.addinitscript", {"surface_id": sid, "script": "window.__termmeshInitMarker = 'init-ok';"})
            c._call("browser.navigate", {"surface_id": sid, "url": second_url})
            _wait_selector(c, sid, "#second", timeout_s=7.0)
            init_value = c._call("browser.eval", {"surface_id": sid, "script": "window.__termmeshInitMarker || ''"}) or {}
            ...
            state_path = tempfile.NamedTemporaryFile(delete=False, prefix="termmesh-state-", suffix=".json").name
