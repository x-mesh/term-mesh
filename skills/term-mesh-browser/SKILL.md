---
name: term-mesh-browser
description: End-user browser automation with term-mesh. Use when you need to open sites, interact with pages, wait for state changes, and extract data from term-mesh browser surfaces.
---

# Browser Automation with term-mesh

Use this skill for browser tasks inside term-mesh webviews.
...
term-mesh browser surface:7 fill e1 "hello"
...
term-mesh browser open https://example.com --workspace workspace:2 --window window:1 --json
```
...
term-mesh supports wait patterns similar to agent-browser:

```bash
term-mesh browser <surface> wait --selector "#ready" --timeout-ms 10000
term-mesh browser <surface> wait --text "Success" --timeout-ms 10000
...
```bash
term-mesh browser open https://example.com/signup --json
term-mesh browser surface:7 snapshot --interactive
term-mesh browser surface:7 fill e1 "Jane Doe"
term-mesh browser surface:7 fill e2 "jane@example.com"
term-mesh browser surface:7 click e3 --snapshot-after --json
term-mesh browser surface:7 wait --url-contains "/welcome" --timeout-ms 15000
term-mesh browser surface:7 snapshot --interactive
```
...
```bash
term-mesh browser surface:7 fill e11 "" --snapshot-after --json
term-mesh browser surface:7 get value e11 --json
```
...
term-mesh browser surface:7 snapshot --interactive
term-mesh browser surface:7 click e5 --snapshot-after --json
term-mesh browser surface:7 wait --load-state complete --timeout-ms 15000
term-mesh browser surface:7 snapshot --interactive
```
