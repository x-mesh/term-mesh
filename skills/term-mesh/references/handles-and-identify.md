...
```bash
term-mesh identify --workspace workspace:2
term-mesh identify --workspace workspace:2 --surface surface:8
```
...
```bash
term-mesh --json identify                 # refs-first output
term-mesh --json --id-format both identify
term-mesh --json --id-format uuids identify
```
