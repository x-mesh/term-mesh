...
--------------------------------------------------------------------------------
1. 배경 및 전략적 포지셔닝
1.1 왜 하이브리드 네이티브인가 (term-mesh 아키텍처 벤치마킹)
...
• 수직형 사이드바 탭 (term-mesh 스타일): 상단 탭 대신 수직형 사이드바를 채택하여 여러 에이전트의 상태(Git 브랜치, 실행 중인 포트 등)를 한눈에 파악.
...
• W2: term-mesh 스타일의 수직형 사이드바 및 네이티브 창 분할(Split) UI 구현.
...
6.5 라이선스
• AGPL-3.0-or-later (term-mesh fork 기반).
