...
termmesh also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If termmesh incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    termmesh_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (termmesh_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing termmesh wrapper .zshenv at {termmesh_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"termmesh_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: termmesh stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["TERMMESH_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(termmesh_wrapper_dir)
        env["TERMMESH_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  termmesh_wrapper_dir={termmesh_wrapper_dir}")
