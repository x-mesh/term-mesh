#!/usr/bin/env python3
"""
Regression: termmesh relies on Ghostty's xterm-ghostty terminfo entry.

In termmesh (embedded GhosttyKit), "bright" SGR 90-97 can render incorrectly
for some palettes. zsh-autosuggestions defaults to `fg=8`, which historically
resolved to SGR 90 via terminfo `setaf`.

termmesh ships a terminfo overlay that forces bright colors to use indexed
