#!/usr/bin/env python3
"""
Regression test: `termmesh --version` should print version text without requiring a socket.
"""
...
def main() -> int:
    ...
    if code != 0:
        print("FAIL: `termmesh --version` exited non-zero")
        print(f"exit={code}")
        print(f"stdout={out}")
        print(f"stderr={err}")
        return 1
    ...
    if not out:
        print("FAIL: `termmesh --version` produced empty stdout")
        return 1
    ...
    if out2 != out:
        print("FAIL: `termmesh --version` and `termmesh version` differ")
        print(f"--version: {out!r}")
        print(f"version:   {out2!r}")
        return 1

    print(f"PASS: termmesh version command works ({out})")
    return 0
