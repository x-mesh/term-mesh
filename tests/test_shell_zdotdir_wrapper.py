...
The termmesh zsh integration sets ZDOTDIR to the app's wrapper directory so zsh
...
def main() -> int:
    ...
    base = Path("/tmp") / f"termmesh_zdotdir_test_{os.getpid()}"
    try:
        if base.exists():
        ...
        (orig / ".zshenv").write_text(
            'echo "$ZDOTDIR" > "$TERMMESH_ZDOTDIR_TEST_OUTPUT"\n',
            encoding="utf-8",
        )

        env = dict(os.environ)
        env["ZDOTDIR"] = str(wrapper_dir)
        env["TERMMESH_ZSH_ZDOTDIR"] = str(orig)
        env["TERMMESH_ZDOTDIR_TEST_OUTPUT"] = str(seen_path)
