...
Run with a tagged instance to avoid unix socket conflicts:
  TERMMESH_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside termmesh and ensure it is actually listening.
...
def main() -> int:
    tag = os.environ.get("TERMMESH_TAG") or ""
    if not tag:
    ...
    try:
        ...
        # Start a listening server outside termmesh. A fresh tab should NOT show this port,
