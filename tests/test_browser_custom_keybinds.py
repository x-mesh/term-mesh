...
def pane_id_for_surface(client: termmesh, surface_id: str) -> Optional[str]:
    ...
    for _surface_index, candidate, pane_id, _title in surfaces:
        if str(candidate) == str(surface_id):
            return str(pane_id)
    return None
...
def test_cmd_ctrl_h_goto_split_left_from_webview(client: termmesh) -> tuple[bool, str]:
    """
...
def test_cmd_opt_left_arrow_goto_split_left_from_webview(client: termmesh) -> tuple[bool, str]:
    """
...
def main() -> int:
    print("termmesh Browser Custom Keybind Tests")
    print("=" * 50)
    client = termmesh()
    client.connect()
