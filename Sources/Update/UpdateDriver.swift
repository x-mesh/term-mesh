...
class UpdateDriver: NSObject, SPUUserDriver {
    ...
    func show(_ request: SPUUpdatePermissionRequest,
        ...
        if (env["TERMMESH_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? env["CMUX_UI_TEST_TRIGGER_UPDATE_CHECK"]) == "1" || (env["TERMMESH_UI_TEST_AUTO_ALLOW_PERMISSION"] ?? env["CMUX_UI_TEST_AUTO_ALLOW_PERMISSION"]) == "1" {
...
#endif
        // Never show Sparkle's permission UI. term-mesh relies on its in-app update pill instead,
    ...
    func showUpdateReleaseNotes(with downloadData: SPUDownloadData) {
        // term-mesh uses Sparkle's UI for release notes links instead.
    }
    ...
    func showUpdateInFocus() {
        // No-op; term-mesh never shows Sparkle dialogs.
    }
