...
/// Attaches the left-hand titlebar controls to every main window.
///
/// The update pill used to ride along as a `.right` accessory. It drew over
/// the custom titlebar strip and took its clicks, so it now lives in that
/// strip as an ordinary row item.
