...
enum RemoteWorkLog {
    ...
    nonisolated static func infoOffMain(_ message: String) { emitOffMain(message, at: .info) }

    /// `debug`, from a thread that is not the main one.
    nonisolated static func debugOffMain(_ message: String) { emitOffMain(message, at: .debug) }
    ...
    nonisolated private static func emitOffMain(_ message: String, at messageLevel: RemoteWorkLogLevel) {
        append("[\(messageLevel.rawValue)] \(message)")
        #if DEBUG
        dlog("remotework.\(message)")
        #endif
