...
final class DiagnosticsRedactor {
    ...
    private var displayNameAliases: [String: String] = [:]
    ...
    func redact(_ text: String) -> String {
        ...
        result = redactingDisplayNames(result)
    ...
    /// Bind a label to a reserved alias. First registration wins when two
    /// hosts share a label; free-text logs cannot distinguish those hosts
    /// either, while the structured peer list still carries each peer alias.
    func register(displayName: String, as alias: String) {
        let key = displayName.trimmingCharacters(in: .whitespacesAndNewlines)
            .precomposedStringWithCanonicalMapping
            .lowercased()
        // A label that already looks anonymous carries no identifying data,
        // and registering it would rewrite aliases generated by this redactor.
        guard !key.isEmpty, !Self.isHostAlias(key),
              displayNameAliases[key] == nil else { return }
        displayNameAliases[key] = alias
    }
    ...
    private static func isHostAlias(_ value: String) -> Bool {
        value.range(of: #"^<host-\d+>$"#, options: .regularExpression) != nil
    }
    ...
    /// Labels run before addresses. A display name may contain its SSH host
    /// (`builder.example.com Mac`); replacing the address first would split the
    /// label and leave the identifying remainder behind.
    private func redactingDisplayNames(_ text: String) -> String {
        var result = text
        for key in displayNameAliases.keys.sorted(by: { $0.count > $1.count }) {
            guard let alias = displayNameAliases[key],
