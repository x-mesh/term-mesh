...
final class DiagnosticsRedactor {
    ...
    private var displayNameRules: [(NSRegularExpression, String)] = []
    ...
    /// Reserve aliases independently of their address. This keeps numbering
    /// tied to peer-list order even for direct-socket hosts with no SSH target
    /// and for duplicate or empty display names.
    func reserveAlias() -> String {
        nextAlias()
    }
    ...
    /// Register a label only as a detector for free-text lines. Partial
    /// replacement is unsafe: `builder` inside `builder.corp.internal` would
    /// expose the domain suffix, and `ci` can be an ordinary word. A matching
    /// line is replaced as a whole with a marker carrying the peer alias.
    func register(displayName: String, as alias: String) {
        let key = displayName.trimmingCharacters(in: .whitespacesAndNewlines)
            .precomposedStringWithCanonicalMapping
            .lowercased()
        // Alias-like labels carry no identifying data and would match the
        // aliases generated in the structured peer section.
        guard key.count >= Self.minimumRedactableNameLength,
              key.unicodeScalars.contains(where: CharacterSet.alphanumerics.contains),
              !Self.isHostAliasLike(key),
              ...
              !displayNameRules.contains(where: { $0.0.pattern == regex.pattern }) else { return }
        displayNameRules.append((regex, alias))
    }

    /// Bind an SSH target to a reserved alias. Address normalization is shared
    /// with `alias(forHost:)`, so `user@host:22` and bare `host` still agree.
    func register(host: String, as alias: String) {
        let key = normalizedHost(host)
        guard !key.isEmpty, aliases[key] == nil else { return }
        aliases[key] = alias
    }

    private func nextAlias() -> String {
        defer { nextHostIndex += 1 }
        return "<host-\(nextHostIndex)>"
    }

    private static func isHostAliasLike(_ value: String) -> Bool {
        value.range(
            of: #"^<?host(?:-\d+)?>?$"#,
...
enum DiagnosticsReport {
    ...
    static func build(
        ...
        safeSnapshot.peerHosts = snapshot.peerHosts.map { host in
            ...
            if let sshTarget = host.sshTarget {
                redactor.register(host: sshTarget, as: alias)
            }
            redactor.register(displayName: host.displayName, as: alias)
