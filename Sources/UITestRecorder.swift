...
/// When `TERMMESH_UI_TEST_KEYEQUIV_PATH` is set, we persist small counters/fields here so tests
/// can assert that menu key equivalents were actually routed and handled.
enum UITestRecorder {
    private static var path: String? {
        let env = ProcessInfo.processInfo.environment
        guard let p = (env["TERMMESH_UI_TEST_KEYEQUIV_PATH"] ?? env["CMUX_UI_TEST_KEYEQUIV_PATH"]), !p.isEmpty else { return nil }
        return p
    }
