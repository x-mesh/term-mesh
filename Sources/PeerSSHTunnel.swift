...
final class PeerSSHTunnel: @unchecked Sendable {
    ...
    private func spawnAttempt(dashboardLocalPort: Int?) async throws {
        ...
        proc.arguments = [
            ...
            "--",
            sshTarget,
            // `-N` owns the forwarding session and does not need a remote
            // command.
        ]
        let errPipe = Pipe()
        let outPipe = Pipe()
        ...
        // is the diagnostic; stdout is drained defensively for SSH configs
        // that enable a command or banner despite the no-command tunnel.
