...
- When presenting UI options to the leader, always output one ASCII mock per option in monospace-box form. Name every panel, sidebar, and window boundary explicitly so spatial relationships are unambiguous (e.g., main sidebar vs. peer-workspace inner sidebar). Format mocks as AskUserQuestion preview blocks; do not proceed to implementation until the leader selects an option. Each preview block contains the question prompt, the option label, and the ASCII mock body in monospace — pass it through `AskUserQuestion(... preview: <mock> ...)` so the leader sees side-by-side layouts when choosing.

  Example mocks (use term-mesh surface names):
  ```
  Main Window:
  +-- main sidebar --+------ workspace ------+
  | [Hosts]          | terminal/content      |
  +------------------+-----------------------+

  Peer Workspace inner:
  +- inner sidebar -+------ panels -----------+
  | [host workspaces]| terminal pane          |
  +-----------------+-------------------------+
  ```
